...
def _chunk_file(file, extension, destination, size, unique_id):
...
def _split(interactive, file, destination, size):
    ...
    if interactive:
        ...
        choices=['floppy', 'zip', 'Fastq', 'Bam']).ask()
        if support == 'floppy':
            size = 1400000
        elif support == 'zip':
            size = 100000000
    ...
    f = Path(file)
    if f.exists():
        unique_id = f.stem  # Get the filename without extension as the unique ID
        with open(f, 'rb') as file_stream:
            _chunk_file(file_stream, f.suffix, destination, size, unique_id)
